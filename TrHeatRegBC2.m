function varargout = TrHeatRegBC2(varargin)
% TRHEATREGBC2 MATLAB code for TrHeatRegBC2.fig
%      TRHEATREGBC2, by itself, creates a new TRHEATREGBC2 or raises the existing
%      singleton*.
%
%      H = TRHEATREGBC2 returns the handle to a new TRHEATREGBC2 or the handle to
%      the existing singleton*.
%
%      TRHEATREGBC2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TRHEATREGBC2.M with the given input arguments.
%
%      TRHEATREGBC2('Property','Value',...) creates a new TRHEATREGBC2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before TrHeatRegBC2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to TrHeatRegBC2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help TrHeatRegBC2

% Last Modified by GUIDE v2.5 17-Nov-2017 17:02:13

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @TrHeatRegBC2_OpeningFcn, ...
    'gui_OutputFcn',  @TrHeatRegBC2_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function varargout = TrHeatRegBC2_OutputFcn(hObject, eventdata, handles) 
...
%% OPENING FUNCTION
% * Executes just before |TrHeatRegBC2| is made visible;
...
function TrHeatRegBC2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to TrHeatRegBC2 (see VARARGIN)

%%
% Creates the |handles| structure
% Choose default command line output for |TrHeatRegBC2|
handles.output = hObject;

%%
% Loads the exterior edges and their types from the previous GUI
load('TrHeatStructDef','edgesArray'); % edgesArray contains all exterior edges
load('TrHeatRegBC1','data','edgesType'); % data contains pairs of edges and their types
...
% |TrHeatRegBC2| file, if such file exists (meaning that the previous GUI
% was left unchanged).
...
if isfield(handles,'edgesDirichlet')==1
    ...
    if exist('TrHeatRegBC2.mat','file') % reuse the previous data
        load('TrHeatRegBC2','dataDir');
...
% created from from scratch or imported from the |TrHeatRegBC2| file, if such 
...
if isfield(handles,'edgesNeumann')==1
    ...
    % If there exists a local |mat| file, it loads it...
    if exist('TrHeatRegBC2.mat','file') % reuse the previous data
        load('TrHeatRegBC2','dataNeu','NeuTime');
...
if isfield(handles,'edgesRobin')==1
    ...
    % If there exists a local |mat| file, it loads it...
    if exist('TrHeatRegBC2.mat','file') % reuse the previous data
        load('TrHeatRegBC2','dataRobin','RobinTime');

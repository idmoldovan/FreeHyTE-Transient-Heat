function PlotFieldsReg(Nodes,Edges,Loops,time,deltaT,SpecOutput,abscissa)
% PLOTFIELDSREG plots the final temperature and flux fields in NGP*NGP
...
if (SpecOutput && ~rem(tstep,SpecOutput)) % if the current step number is a
    ...
    for ii=1:length(Loops.area)
        ...
        % direction. sze simply collects the distances between the two most
        % far apart points of the element in x and y directions. ASSUMES
        % THAT THE ELEMENT IS RECTANGULAR!!
        sze = max(Nodes(LocLoop.nodes(:),:))-min(Nodes(LocLoop.nodes(:),:));
        
        % Generating the output grid
        [x,y] = ndgrid(1/2*sze(1)*abscissa,1/2*sze(2)*abscissa);
        GlobalX = LocLoop.center(1)+x;
        GlobalY = LocLoop.center(2)+y;

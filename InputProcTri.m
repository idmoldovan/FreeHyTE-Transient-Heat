function [NGP, Nodes, Edges, Loops, BConds, RegCode, SpecOutput] = InputProcTri
% INPUTPROCTRI is the input processing function for irregular (triangular)
% meshes. 
%
...
% 4. Coutinho, AP,  Elementos finitos híbridos-Trefftz para problemas de
% temperatura em regime transitório. MSc Thesis, Universidade Nova de
% Lisboa, 2017 (in Portuguese).
%
% INPUT DATA (read from the *.mat files created by the GUIs):
% * from TrHeatStructDef: SpecOutput (number of steps to plot the 
...
% * from TrHeatTriBC1: nodes, edges_nodes, edges_loops, loops_nodes and 
...
% * from TrHeatTriBC2: edgesDirichlet, edgesNeumann, edgesRobin, dataDir, 
% dataNeu, dataRobin ;
...
% Loads mesh information from STRUCTDEF GUI
load('TrHeatStructDef','SpecOutput');
...
Loops.material(:,4) = k/c/rho; %thermal diffusity (alpha)
...
load('TrHeatTriBC2','edgesDirichlet','edgesNeumann','edgesRobin',...
...
% ... or the same process, one edge at a time 
% Edges.order(13) = 3;
% Edges.order(17) = 3;
...
Loops.gc(:) = ceil(sqrt(3*(2*LoopsOrderP+1))); % number of Gauss collocation points in each direction
% 2. Complementary solution basis
% order of the complementary solution basis - uniform
Loops.orderC(:) = LoopsOrderC; 
% ...or non-uniform & one at a time
% Loops.order(1,1) = 5;
% Loops.order(1,2) = 5;
% Loops.order(1,3) = 5;

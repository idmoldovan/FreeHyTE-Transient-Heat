...
% MainTri is the main routine of FreeHyTE - Transient Heat Conduction 
...
%
%% ---------- User input ------------
% Launch pre-processing routine: INPUTPROCTRI and TIMEGENTRI
...
% colour maps.
%
[NGP,Nodes,Edges,Loops,BConds,RegCode,SpecOutput] = InputProcTri; %ILDI
%
% Launch time domain data processor. TIMEGENTRI computes the generalized
...
% heat source (InternHeat). The other output of TIMEGENTRI is listed below:
...
%
Loops = ComputeCollocationInitialFieldsTri(Nodes,Loops,Ini);
%
% Computing the initial fields in the Gauss points, for plotting
%
Loops = ComputeGaussInitialFieldsTri(Nodes,Loops,Ini,NGP);
%
...
for time=deltaT:deltaT:TotalTime
  ...
  %% --------- Computation of the particular solution ------------
    ...
    % solving system.
    % 
    PartLHS = Gen_LHS_MatrixTri(Nodes,Loops,DimPart,RegCode);
    %
    % Generating the particular RHS vector with the source term
    %
    PartRHS = Gen_RHS_VectorTri(Nodes,Loops,InternHeat,DimPart,...
        time,deltaT,theta);
    %    
    ...
    %
    [Loops] = ComputeCollocationFieldsTri(Nodes,Loops,X,Xp,deltaT,theta);
    %
    % Preparing the fields for plotting and/or storing
    %
    [Loops,SubDir] = ComputeGaussFieldsTri(Nodes,Loops,X,...
            Xp,time,deltaT,SubDir,SpecOutput,NGP,theta); 
    %
    % Plotting the displacement field
    %
    PlotFieldsTri(Nodes,Edges,Loops,time,deltaT,SpecOutput,NGP);        
end
%% **********************************************************************

function [Loops,SubDir] = ComputeGaussFieldsReg(Nodes,Loops,X,...
            Xp,time,deltaT,SubDir,SpecOutput,abscissa,theta)
% COMPUTEGAUSSFIELDSREG  computes the final temperature and flux fields, 
...
% the user (SpecOutput).
%
% COMPUTEGAUSSFIELDSREG is called by MAINREG. 
% Input:
...
% results should be stored, the Gauss abscissas and the time discretization
% parameter theta.
% Besides the input arguments,  COMPUTEGAUSSFIELDSREG loads from
...
% (NGP+1)^2 plotting points, in the Loops structure returned to MAINREG, 
...
if ~isempty(DirName)  
    ...
    % results 
    if tstep == 1 
        % creates a sub-folder in the DirName folder with the FileName name
    ...
    % followed by the values of the temperature and flux fields (complex
    % numbers). 
...
for ii=1:length(Loops.area)   
    ...
    % Generating the local x and y coordinates of Gauss collocation points 
    ...
    T = reshape(T,[length(abscissa),length(abscissa)]);
    V = reshape(V,[length(abscissa),length(abscissa)]);
    ...
    Loops.Qx(ii) = {zeros(length(abscissa))};
    Loops.Qy(ii) = {zeros(length(abscissa))};
    Qx = cos(t(:)).*qr - sin(t(:)).*qth;
    Qy = sin(t(:)).*qr + cos(t(:)).*qth;
    Qx = reshape(Qx,[length(abscissa),length(abscissa)]);
    Qy = reshape(Qy,[length(abscissa),length(abscissa)]);
        ...
        for jj = 1:length(abscissa)
            for kk = 1:length(abscissa)
                fprintf(FileU,'%0.6e %0.6e %0.6e %0.6e %0.6e %0.6e %0.6e %0.6e\n',...
                    globalx(jj,kk), globaly(jj,kk), real(T(jj,kk)), imag(T(jj,kk)),...
                    real(Qx(jj,kk)), imag(Qx(jj,kk)), ...
